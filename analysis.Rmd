...
for (j in 1:length(study_name)) {
  ...
  AI_tmp<-readNifti(file.path(data_path,jac_type, 'vol_AI', glue('{vol_AI_file}_01_SE01_MR.nii.gz')))
  ...
  var_ratio_img %>% asNifti %>% writeNifti(glue('asset/nii/{jac_type}_{study_name[j]}_var_ratio'), template = vol_template)
  var_sign_img %>% asNifti %>% writeNifti(glue('asset/nii/{jac_type}_{study_name[j]}_var_sign'), template = vol_template)
  hedges_g_img %>% asNifti %>% writeNifti(glue('asset/nii/{jac_type}_{study_name[j]}_hedges_g'), template = vol_template)
  hedges_sign_img %>% asNifti %>% writeNifti(glue('asset/nii/{jac_type}_{study_name[j]}_hedges_sign'), template = vol_template)
